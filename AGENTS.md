...
- `cmd/dataforseo/`: CLI entrypoint
- `internal/`: implementation packages
  - `cmd/`: Kong CLI commands
  - `api/`: REST client
  - `dataforseo/`: DataForSEO API client and types
...
- `make` / `make build`: build `bin/dataforseo-cli`
...
- Formatting: `make fmt` (`goimports` local prefix `github.com/builtbyrobben/dataforseo-cli` + `gofumpt`)
